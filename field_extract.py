...
def get_keyword(keyword, look_for):
    if look_for == "is_counter":
        if (
            keyword in ["+CASSETTE", "CASSETTE", "SETTE", "CASS", "CST"]
            or "CASSETTE" in keyword
        ):
            return "CASSETTE"
        if (
            keyword in ["+REJECTED", "REJECTED", "+REJ", "REJ", "PURG"]
            or "REJECTED" in keyword
        ):
            return "REJECTED"
        if (
            keyword
        ...
        if (
            ...
            or "DISPENSED" in keyword
        ):
            return "DISPENSED"
        if keyword in ["=TOTAL", "TOTAL"] or "TOTAL" in keyword:
            return "TOTAL"

    if look_for == "is_switch":
        if keyword in ["STR", "ST"]:
            return "STR"
        if keyword in ["INC", "IC"]:
            return "INC"
        if keyword in ["OUT", "OU", "OC"]:
            return "OUT"
        if keyword in ["DEC", "DC"]:
            return "DEC"
        if keyword in ["END", "EC"]:
            return "END"
    ...
    for rows in counter_values:
        ...
        for row in rows:
            ...
            if keyword in ["CASSETTE", "REJECTED", "REMAINING", "DISPENSED", "TOTAL"]:
                ...
                after_keyword = True
                if not keyword in keyword_seen:
...
def is_triangulation_pass(resp, look_for):
    ...
    if look_for in [
        ...
        "Counter#2",
        "Counter#3",
    ]:  # means we are looking in counter
        return (
            resp["STR"]["value"]
            + resp["INC"]["value"]
            - resp["DEC"]["value"]
    ...
    # for counter
    if look_for in ["TYPE1", "TYPE2", "TYPE3", "TYPE4"]:
        return (
            resp["REMAINING"]["value"]
            == resp["CASSETTE"]["value"] + resp["REJECTED"]["value"]
            and resp["REMAINING"]["value"] + resp["DISPENSED"]["value"]
            == resp["TOTAL"]["value"]
        )
        ...
        if keyword in ["STR", "INC", "DEC", "OUT", "END"]:
            if index + 2 < len(switch_val):
                ...
                # handles value that has some alpha character from background
                    ...
                    not temp.isalpha()
                    and temp.isalnum()
                    and not (len(temp) == 2 and temp[0] == "C")
                ):
                ...
                if look_for == "C1":
                    switch_resp["Counter#0"][keyword] = {
                        "value": value,
                        "value_co_ord": value_co_ords,
                        "key_co_ords": key_co_ords,
                    }
                elif look_for == "C2":
                    switch_resp["Counter#1"][keyword] = {
                        "value": value,
                        "value_co_ord": value_co_ords,
                        "key_co_ords": key_co_ords,
                    }
                elif look_for == "C3":
                    switch_resp["Counter#2"][keyword] = {
                        "value": value,
                        "value_co_ord": value_co_ords,
                        "key_co_ords": key_co_ords,
                    }
                elif look_for in ["C4", "C5"]:
                    switch_resp["Counter#3"][keyword] = {
                        "value": value,
                        "value_co_ord": value_co_ords,
                        "key_co_ords": key_co_ords,
                    }
...
def extract_fields(ocr_resp):
    ...
    result = {
        "counter1": counter1_resp,
        "counter2": counter2_resp,
        "switch1": switch1_resp,
        "switch2": switch2_resp,
    }
    return result
